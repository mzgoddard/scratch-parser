 ...
 * @return {void}
 */
module.exports = function (rawInput, isSprite, callback) {
    if (typeof rawInput === 'string') {
        // Pass string to callback
        return callback(null, [rawInput, null]);
    }
    ...
    if (!Buffer.isBuffer(input)) {
        try {
            if (input.slice) {
                input = Buffer.from(input.slice(0, 3));
            } else {
    ...
    if (!isZip && !isLegacy && !isGZip) {
        try {
            return callback(null, [new TextDecoder().decode(rawInput), null]);
        } catch (decodeError) {
            return callback(null, [Buffer.from(rawInput).toString('utf-8'), null]);
        }
    }
    ...
    unzip(Buffer.from(rawInput), isGZip, isSprite, callback);
};
